...
export const addToIpfs = async (obj: Record<string, any>): Promise<AddResult> => {
    const result = await ipfsClient.add(JSON.stringify(obj));
    return result;
};

export const getMetadata = async(ipfsHash: string) => {
    return getNFTMetadataFromIPFS(ipfsHash);
};
